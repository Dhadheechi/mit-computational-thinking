...
Text(String(rand(alphabet, 40))) # the text function just removes the quotations
...
# ╔═╡ ba368228-ced5-4ce5-8f5a-ddccd09b4551
lowercase("Abosgb") # also works for a string directly; so i guess we don't need to use a map: 
...
unaccent(french_word) # this is a function that directly applies to a word, so we don't need to use a map, unlike in lowercase
...
function clean(text)
	unaccented = unaccent(text)
	lower = map(lowercase, unaccented)
	return filter(isinalphabet, lower)
end

# ╔═╡ e00d521a-f992-11ea-11e0-e9da8255b23b
clean("Crème brûlée est mon plat préféré.")

# ╔═╡ 03fa8ae0-fe1e-4828-8bbd-7ccae5b42cc0
unaccent("Crème brûlée est mon plat préféré.")
...
function letter_frequencies(txt)
	...
	f = count.(string.(alphabet), txt) # apply the count function to each "string"
	f ./ sum(f)
end

# ╔═╡ e14e6612-a536-44b5-88b2-1a2a7d3bf0de
string.(alphabet)
...
unused_letters = [alphabet[index] for index in index_of_letter.(alphabet) if sample_freqs[index] == 0.0] # replace with your answer
...
function rand_sample(frequencies) #works for a general frequency list, not just 0...1
	x = rand()
	findfirst(z -> z >= x, cumsum(frequencies ./ sum(frequencies))) # returns index
end

# ╔═╡ ac7fba2e-60f7-4bb2-a2c3-d4d20cffd834
md"The above function finds the first *index* z such that the letter at index z has a greater cumulative frequency than the random number x. This procedure generates a random letter given the frequency list."

# ╔═╡ 298182f0-862a-4747-8c9f-5b1974930f19
cumsum(sample_freqs./sum(sample_freqs))

# ╔═╡ 0e872a6c-f937-11ea-125e-37958713a495
function rand_sample_letter(frequencies)
	alphabet[rand_sample(frequencies)] # rand_sample generates an *index*
end
...
transition_frequencies = normalize_array ∘ transition_counts; # a function
...
# ╔═╡ 69dfd021-a700-4bb5-b726-417e5fccb504
ee_frequency = sample_freq_matrix[index_of_letter('e'), index_of_letter('e')]
...
double_letters = [
	...
	if sample_freq_matrix[index_of_letter(letter), index_of_letter(letter)] != 0] # replace with your answer
...
begin
	...
	for letter in alphabet
		freq = sample_freq_matrix[index_of_letter('w'), index_of_letter(letter)]
		if freq > max_freq
			max_freq = freq
			most_likely_to_follow_w = letter
		end
	end
		
end

# ╔═╡ aebc9256-ed50-4bc4-9d38-ae9e2ff2ca4e
most_likely_to_follow_w
...
begin
	...
	for letter in alphabet
		freq = sample_freq_matrix[index_of_letter(letter), index_of_letter('w')]
		if freq > max_freq_precede
			max_freq_precede = freq
			most_likely_to_precede_w = letter
		end
	end
	most_likely_to_precede_w
end
...
row_sums = [sum(sample_freq_matrix[row, :]) for row in 1:size(sample_freq_matrix)[1]]


# ╔═╡ 4a0314a6-7dc0-4ee9-842b-3f9bd4d61fb1
col_sums = [sum(sample_freq_matrix[:, column]) for column in 1:size(sample_freq_matrix)[2]] 

# ╔═╡ cc62929e-f9af-11ea-06b9-439ac08dcb52
row_col_answer = md"""
ith row represents the fraction of all bigrams starting with the letter i.
jth column represents the fraction of all bigrams ending with the letter j
"""
...
# ╔═╡ e6ae5851-2074-4d45-8a72-025907c3450c
length(emma_words)
...
# ╔═╡ 4da2259f-dc7b-406e-8420-57bf580e5341
md"The anonymous function inside the 'do' block returns two consective words, which is then applied to each of the n-1 starting positions of the text 'words'"
...
function ngrams(words, n)
	starting_positions = 1:length(words)-(n-1) # only till the nth word from the end
...
function word_counts(words::Vector)
	...
	for word in words
		if haskey(counts, word)
			counts[word] += 1 # increasing the count of an existing word
		else 
			counts[word] = 1 # creating the key for a new word in the dictionary
		end
	end
...
function completion_cache(grams)
	...
	for gram in grams
		n = length(gram)
		n_minus_one = gram[1:(end-1)]
		nth = gram[end]
		if haskey(cache, n_minus_one)
			push!(cache[n_minus_one], nth)
		else
			cache[n_minus_one] = [nth]
		end
	end
	cache
end
...
function generate_from_ngrams(grams, num_words)
	...
	for i ∈ n+1:num_words
		...
		# possible next words
		completions = cache[tail] # all possible words ending the tail
...
end # what's the point of this function?
...
# ╔═╡ c82853c6-3117-4fad-9eec-4680cfa4011e
# RGB(0.36, 0.82, 0.8)
