...
# ╔═╡ d9d25046-3beb-4271-ac35-7c99f6d408dd
md"The reasoning is as follows: We have a point x0 which we consider as a good first guess to the root. Say x1 = x0 + δ is the actual root. We don't know either x1 or δ. So we write f(x0 + δ) = 0, and since δ is small, we can approximate f(x0 + δ) by a linear function f(x0) + δf′(x0). And equating this approximation to 0 gives us an *approximate* value of δ, which we can use to get an approximate x1, and repeat this process until we actuallly converge to the root."
...
function newton1D(f, x0) # finds a root for f, given an initial guess x0
...
md"""
We want to find the inverse $T^{-1}(y)$, i.e. to solve the equation $T(x) = y$ for $x$. We know $y$.
...
where $J := DT_{x_0}$ is the Jacobian matrix of $T$ at $x_0$, i.e. the best linear approximation of $T$ near to $x_0$. We need to find the step $δ$.
"""
...
function newton2D_step(T, x)
	...
	δ = J \ T(x)   # J^(-1) * T(x). solves J.δ = T(x)
	
	return x - δ
end

# ╔═╡ 923bde64-7ba4-11eb-21e9-a11993aaab2e
"Looks for x such that T(x) = 0, i.e., finds a root. "
...
# ╔═╡ 57f40952-4d9c-46bc-baeb-cfde6aaaf0bc
md"The reasoning is as follows. We have the function f, and an *output* y. We want to find the x where it came from, so that f(x) = y. Now, write this equation as f(x)-y = 0. Say this equation is some T(x) = 0. Now the produces reduces to finding a root for T(x). So we apply the newton's method here, i.e., take an initial guess x0 = [0,0], find T(x0 + δ), assume it is equal to 0, write the linear approximation of T(x0+δ) as T(x) + J*δ, where J is the Jacobain of T(x) evaluated at x=x0, and δ is the two-dimensinal step that we'd like to find. We get δ = J \ T(x), from which we get x1 = x0 + δ, and we iterate further. "
...
jacobian(T(p), [a, b]) .|> Text # is the identity when p = 0
...
expand( f(z + η) ) - ( f(z) + η*f′(z) ) # contains all of the non-linear terms in η
...
simplify.(expand.(image - T(p)([a, b]) - jacobian(T(p), [a, b]) * [δ, ϵ])) # will contain only the non-linear terms in ϵ and δ
...
manifest_format = "2.0"
project_hash = "f3d0c7b3f276d943418c7b4f77340ab3fda5e9bd"
