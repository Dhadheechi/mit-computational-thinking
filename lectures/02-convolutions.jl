...
kernel = Kernel.gaussian((2, 2)) # 3 standard deviations on either axis

# ╔═╡ a9b8c639-707a-4255-ba41-1e65ca6d458b
kernel2 = centered([
	-2.5 -3.0 -2.5
	-3.0  23.0 -3.0
	-2.5 -3.0 -2.5
...
# ╔═╡ 5af3cf6a-e3b3-4d12-b8e4-8e9808404a96
kernel3 = centered([
	-0.5 -1.0 -0.5
	-1.0 7.0 -1.0
	-0.5 -1.0 -0.5
...
# ╔═╡ 6f35f7d7-3819-40f0-8bb0-1294a580b02d
# kernel2_normalized = kernel2 ./ maximum(abs.(kernel2))

# ╔═╡ e857632c-64e9-4938-a822-33316bad84b0
sobel = Kernel.sobel()[2]

# ╔═╡ 80d36f9b-b392-43c4-8ea7-e5f07f73e082
sum(kernel2)

# ╔═╡ 15a6097c-20f5-426d-b224-2c3272c02261
sum(kernel3)

# ╔═╡ 28b2b4d7-ed12-49d2-84f0-9a61a4556901
load("bowtie.jpg")
...
function show_colored_kernel(kernel)
	kernel = kernel ./ maximum(abs.(kernel))
	print(kernel)
	to_rgb(x) = RGB(max(-x, 0), max(x, 0), 0)
	to_rgb.(kernel) #/ maximum(abs.(kernel))
end

# ╔═╡ dd550b50-62b3-44cb-a4f6-032d20e0c642
show_colored_kernel(sobel)
...
# ╔═╡ 85025541-a4ef-44a0-a5dc-08331f81a888
function convert_to_float64(img::Matrix{RGBA{N0f8}})
    return RGBA{Float64}.(img)  # Broadcast conversion
end

# ╔═╡ 6d39fea8-ed3c-11ea-3d7c-3f62ca91ce23
begin
	large_image = load("bowtie.jpg")
	large_image = convert_to_float64(large_image) # the image doesn't get as sharpened as before
...
# ╔═╡ 13033f65-7de8-4a00-8f72-9f8b8b8566cd
convolve(large_image, sobel)
# large_image

# ╔═╡ 32ec3008-ec60-4ee0-9fb9-02c2456f46c1
convolve(image, kernel3) # sharpened too much

# ╔═╡ 5bebc1e8-8123-400d-bceb-81583401a2fa
large_image

# ╔═╡ 14abf701-0ce7-4efe-b201-2cca468b1ea7
20*Gray.(abs.(convolve(large_image, sobel))) # I am having to multiply the image by a much greater constant (20 as compared to Grant's 3) to notice the edge detection
...
conv_image = convolve(large_image, kernel2)

# ╔═╡ 6340c0f8-ee18-11ea-1765-45f4bc140670
heatmap_2d_fourier_spectrum(conv_image)

# ╔═╡ c41d498b-2917-4991-bbc7-8f3eee3c80ec
typeof(image)
...
manifest_format = "2.0"
project_hash = "8bb46d7503cdb1c6db5cf755ce8693707e36a2a1"
