...
md"If you look at the computed colored result of the convolution, i.e., the image that looks like (), you can see that the right part is bright, i.e, the convolution has a positive value at that part. But if you see the kernel, it is like [- 0 +]. That is, it should detect a positve change when the image moves from a dark part to a bright part. But in the original image, the positive part of the convolution correpsonds to a change from brighter part to a darker part (white to blue). The reason for this is because the kernel is *flipped*, i.e., reflected, around the middle element before performing the convolution. This way, the definition of convolution in image processing is consistent with that in mathematics (eg. polynomial multiplication."
